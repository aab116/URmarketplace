...
public class URMarketplaceConsole {
    ...
    private void doLogin() {
        ...
        if (user != null) {
            ...
            System.out.println("Incorrect login. Check your username and password.");
    }
    ...
    private void lookForItems() {
        ...
        for (Listing l : all) {
            System.out.println(
                    "[" + l.getId() + "] " + l.getTitle() + " - $" + l.getCurrentBid() + " (" + l.getStatus() + ")");
        }
    }
    ...
    private void postItem() {
        ...
        int listingId;
        if (buyNowPrice > 0)
            listingId = biddingManager.createListingWithBuyNow(title,
                    description,
            ...
            listingId = biddingManager.createListing(title,
                    description,
                    currentUser.getUsername(),
                    price,
                    category,
        ...
        ((TrackOwnerAndCalculateFees) feeCalculator).recordOriginalOwner(String.valueOf(listingId),
                currentUser.getUsername());
    ...
    private void makeOffer() {
        ...
        if (status > 0)
            ...
            System.out.println("The listing does not exist.");
        else if (status == -2)
            System.out.println("The Listing is not active.");
        else if (status == -3)
            System.out.println("The Bid is too low. It must be higher than current bid.");
    }
    ...
    private void buyNow() {
        ...
        if (success && purchasedItem != null) {
            ...
            System.out.println(
                    "I'm sorry, but your purchase has failed. The Item \n - May not be active \n - Doesn't contain a buy now option");
        }
    }
    ...
    private void calculateFees() {
        ...
        // Find the listing to get the sale price and seller
        Listing target = null;
        for (Listing l : biddingManager.getAllListings()) {
        ...
        // Pull the real active storage fee for this listing
        double storageFee = arrangeStorage.getActiveStorageFeeForItem(calcId);

        // Use the actual seller username, not the currently logged-in user
        FeeCalculator.FeeResult result = feeCalculator.calculateFees(
                calcId,
                target.getSellerUsername(),
                target.getCurrentBid(),
                storageFee);
        ...
        System.out.println(
                "Net Payout to " + result.originalOwnerUserId + ": $" + String.format("%.2f", result.netToSeller));
    }
